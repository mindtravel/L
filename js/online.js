...
(function (root) {
  'use strict';
  var ZJ = root.ZJ = root.ZJ || {}, socket = null, player = null, roomId = null, token = null, version = -1, pending = null, reconnectTimer = null;
  ...
  function handle(msg) {
    ...
    if (msg.token) { token = msg.token; try { root.localStorage.setItem('l-room-' + roomId, token); } catch (_) {} }
  ...
  function connect() {
    ...
    socket.onclose = function () {
      status('连接已断开');
      if (roomId && token && ZJ.game && ZJ.game.cfg.mode === 'online') {
        clearTimeout(reconnectTimer);
        reconnectTimer = setTimeout(function () { connect(); pending = { type: 'join', roomId: roomId, token: token }; }, 1200);
      }
    };
  ...
  ZJ.online = {
    ...
    join: function (id) {
      connect(); var room = String(id || '').trim().toUpperCase(), saved = null;
      try { saved = root.localStorage.getItem('l-room-' + room); } catch (_) {}
      var m = { type: 'join', roomId: room }; if (saved) m.token = saved;
      if (socket && socket.readyState === 1) send(m); else pending = m;
    },
